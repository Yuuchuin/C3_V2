...
def vis_results(exp_name, epoch, writer, restore, img, pred_map, gt_map):
    ...
    pred_map = F.interpolate(pred_map, [h,w])
    gt_map = F.interpolate(gt_map, [h, w])
...
def game(den, gt, l, factor):
    w, h = den.shape[-2:]
    step_w = torch.linspace(0, w, 2**l + 1)
    step_h = torch.linspace(0, h, 2**l + 1)
    ...
    # 逐图片地
    for w,_ in enumerate(step_w[0:-1]):
        for h,_ in enumerate(step_h[0:-1]):
            den_sum = torch.sum(den[0][int(step_w[w]):int(step_w[w+1]), int(step_h[h]):int(step_h[h+1])]) / factor
            #print(w,w+1,h,h+1)
            gt_sum = torch.sum(gt[int(step_w[w]):int(step_w[w+1]), int(step_h[h]):int(step_h[h+1])]) / factor
            errors.update(abs(den_sum - gt_sum))
            #print(w, h, den_sum, gt_sum)
            #break


    error = errors.avg * 4**l
    return error
