...
class SCC_BaseModel(nn.Module):
    def __init__(self, dataloader, cfg, cfg_data, pwd):
        super(SCC_BaseModel, self).__init__()
    ...
    def trainer(self):
        for epoch in range(self.epoch, cfg.MAX_EPOCH):
            ...
            if epoch % cfg.VAL_FREQ == 0 or epoch > cfg.VAL_DENSE_START:
                ...
                if self.data_set in ['SHHA', 'SHHB', 'QNRF', 'UCF50']:
                    self.validate_V1()
                elif self.data_set is 'WE':
                    self.validate_V2()
                elif self.data_set is 'GCC':
                    self.validate_V3()
