...
class M2T2OCC_BaseModel(nn.Module):
    def __init__(self, dataloader, cfg, cfg_data, pwd, loss_1_fn,loss_2_fn):
        super(M2T2OCC_BaseModel, self).__init__()
    ...
    def predict(self, img, gt_map=None, gt_cls_label=None):
        density_map, density_cls_score = self.net(img)

        # pdb.set_trace()

        density_cls_prob = F.softmax(density_cls_score, dim=1)

        self.loss_mse, self.cross_entropy = self.build_loss(density_map.squeeze(), gt_map.squeeze(), density_cls_prob,
                                                            gt_cls_label)
        return density_map
    ...
    def train(self):  # training for all datasets
        ...
        for i, data in enumerate(self.train_loader, 0):
            ...
            pred_map = self.predict(img, gt_map, gt_label)
            ...
            if (i + 1) % cfg.PRINT_FREQ == 0:
                ...
                print('[ep %d][it %d][loss %.8f, %.8f, %.8f][lr %.4f][%.2fs]' % \
                      (self.epoch + 1, i + 1, loss.item(), loss1.item(), loss2.item(),
            ...
            for vi, data in enumerate(i_loader, 0):
                ...
                with torch.no_grad():
                    ...
                    for i_img in range(pred_map.shape[0]):
                        ...
                        losses.update(self.loss.item(), i_sub)
    ...
    def test(self, state_path):  # test for SHHA, SHHB, UCF-QNRF, UCF50
        print(state_path)
        self.net.eval()
        ...
        for ti, data in enumerate(self.val_loader, 0):
            img, gt_map = data
