...
def loading_data(train_mode):
    ...
    train_loader = None
    if train_mode == 'DA':
        train_set = UCSD(cfg_data.DATA_PATH+'/train', 'train',main_transform=train_main_transform, img_transform=img_transform, gt_transform=gt_transform)
    ...
    test_set = UCSD(cfg_data.DATA_PATH + '/test', 'test', main_transform=None, img_transform=img_transform,
                   gt_transform=gt_transform)
    test_loader = DataLoader(test_set, batch_size=cfg_data.TEST_BATCH_SIZE, num_workers=0, shuffle=False, drop_last=False)
